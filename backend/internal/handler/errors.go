...
func handleError(c *gin.Context, err error) {
	...
	case errors.Is(err, repository.ErrDuplicateSplit):
		util.Fail(c, http.StatusConflict, "duplicate_split", err.Error())
	case errors.Is(err, repository.ErrInsufficientQuantity):
		util.Fail(c, http.StatusUnprocessableEntity, "insufficient_quantity", err.Error())
